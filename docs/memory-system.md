...
首版不做定时任务，只提供**手动记忆抽取**按钮：

- 在主界面增加一个按钮，例如“记忆提取”；
- 点击后弹出一个文本框（或使用浏览器原生 `prompt`）：
  - 用户可以粘贴一段对话/摘要/文本；
  - 前端调用 `pywebview.api.extract_memory(text)`；
  - 后端使用 `extract_and_store_from_text` 抽取并保存；
  - 前端在消息区显示结果概览，例如：
    - “已提取记忆：事实 2 条，事件 1 条，关系 0 条。”

### 4. 定时任务（后续版本）

未来在 v0.2+ 中，可以增加后台定时任务：

- 每隔一段时间扫描会话历史；
- 对“距离当前有一段时间且尚未抽取”的历史片段进行自动记忆抽取；
- 与手动入口共存（手动入口适合选定特定文本）。

首版先通过**手动入口**验证记忆结构和抽取质量，再逐步演进到自动化。  

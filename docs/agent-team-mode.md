...
- 记忆系统（[记忆设计 v1.0](AI智能体ruyi72%20记忆系统（永驻+事件）设计（v1.0）.md) / [v2.0 目标](AI智能体ruyi72%20记忆系统（永驻+事件）设计（v2.0）.md)）与团队模式**正交**：团队会话可选择是否沿用全局记忆注入策略；若注入，建议在**仅第一条用户消息**或**每一轮仅 A1** 注入，避免重复 token。
...
以下为对记忆设计文档成文的**对照审查**结论，已吸收进正文或本节（详见 [记忆设计 v1.0](AI智能体ruyi72%20记忆系统（永驻+事件）设计（v1.0）.md) 当前版本；演进目标见 [v2.0](AI智能体ruyi72%20记忆系统（永驻+事件）设计（v2.0）.md)）。

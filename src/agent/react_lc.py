...
# 无 JSON citations 时，仅从正文扫 URL（无偏移）；避免 read_file 等工具误报。
_TOOL_URL_FALLBACK_NAMES = frozenset(
    {"search_history", "search_memory", "search_memory_semantic", "browse_memory"}
)

_FENCED_JSON = re.compile(r"```(?:json)?\s*([\s\S]*?)```", re.IGNORECASE)


def _normalize_tool_citation_row(item: dict[str, Any]) -> dict[str, Any] | None:
    url = str(item.get("url") or "").strip()
    if not url:
        return None
    row: dict[str, Any] = {"url": url, "title": str(item.get("title") or "")[:300]}
    try:
        s = int(item.get("start", -1))
        e = int(item.get("end", -1))
        if s >= 0 and e > s:
            row["start"] = s
            row["end"] = e
...
def _citation_rows_from_tool_text(text: str) -> list[dict[str, Any]]:
    """从工具输出中解析 citations（整段 JSON 或 fenced ```json 块）。"""
    raw = (text or "").strip()
    if not raw:
        return []
    ...
    def _from_obj(obj: Any) -> list[dict[str, Any]]:
        if not isinstance(obj, dict):
            return []
        lst = obj.get("citations")
        if not isinstance(lst, list):
            return []
        out: list[dict[str, Any]] = []
        for x in lst:
            if isinstance(x, dict):
                n = _normalize_tool_citation_row(x)
                if n:
                    out.append(n)
        return out
    ...
    try:
        obj = json.loads(raw)
        rows.extend(_from_obj(obj))
    except json.JSONDecodeError:
        pass
    if not rows:
        for m in _FENCED_JSON.finditer(raw):
...
def collect_citation_rows_from_agent_messages(msgs: list[BaseMessage]) -> list[dict[str, Any]]:
    """
    取「最后一条 AIMessage」之前出现的 ToolMessage：优先解析 citations JSON；
    若无有效条目且工具名为记忆/检索白名单，则从正文提取 URL（无偏移）。
    按 url 去重后合并，供 output_review 的 extend_citations_from_tools 使用。
    """
    last_ai = -1
    ...
    seen: set[str] = set()
    out: list[dict[str, Any]] = []
    for i in range(last_ai):
        if not isinstance(msgs[i], ToolMessage):
            continue
        tm = msgs[i]
        content_str = str(tm.content or "")
        rows_for_tool: list[dict[str, Any]] = _citation_rows_from_tool_text(content_str)
        if not rows_for_tool:
            tname = (getattr(tm, "name", "") or "").strip()
            if tname in _TOOL_URL_FALLBACK_NAMES:
                # 延迟导入，避免 service 包 → conversation → react 与 react_lc 顶层循环
                from src.service.output_review_sync import extract_urls_as_tool_citation_rows

                rows_for_tool = extract_urls_as_tool_citation_rows(content_str)
        for row in rows_for_tool:
...
def _attach_tool_citations_to_last_assistant(
    flat: list[dict[str, Any]], rows: list[dict[str, Any]]
) -> None:
    if not rows:
        return
    for i in range(len(flat) - 1, -1, -1):
        if flat[i].get("role") == "assistant":
            flat[i]["tool_citations"] = rows
            return
...
- search_memory_semantic：对事实与事件做向量语义检索（需 memory.vector_enabled 且 Ollama）；可选包含虚构事件向量。
...
def _make_tools(
    ...
    *,
    ...
    def search_memory_semantic(
        ...
        """语义相似检索：事实向量 +（默认）事件向量。include_fictional_events 为 True 时事件命中可含虚构叙事。"""
        return run_search_memory_semantic(
            query,
            top_k=top_k,
            include_events=include_events,
            include_fictional_events=include_fictional_events,
        )
...
def run_scheduler_safe_agent(
    ...
    *,
    ...
    _attach_tool_citations_to_last_assistant(
        flat, collect_citation_rows_from_agent_messages(raw_msgs)
    )
...
def _display_trace(msgs: list[dict[str, Any]]) -> str:
    if not msgs:
        return ""
    ...
    *,
    ...
    if interrupted:
        ...
        if isinstance(raw_i, list):
            ...
            _attach_tool_citations_to_last_assistant(
                flat_i, collect_citation_rows_from_agent_messages(raw_i)
            )
            messages.clear()
